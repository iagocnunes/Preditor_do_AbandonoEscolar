...
clear all
set more off
use "$out\base 6EF ano `x'.dta", clear
...
drop _freq dup
save "$out\\inep com 2 idaluno 6EF `x'", replace
restore
preserve
keep if codigo_inep == .
...
tab dup
drop rnd n
save "$out\\Base 6EF `x' Sem Duplicados", replace
}
local ano 2019 2020
foreach x of local ano {
clear all
use "$out\\Base 6EF `x' Sem Duplicados", clear
tab disciplina
...
sort idaluno
save "$out\\base 6EF `x'_`l'", replace
restore
}
use "$out\\base 6EF `x'_cien", clear
foreach l in  "geo" "his" "lp" "mt"{
merge 1:1 idaluno using "$out\\base 6EF `x'_`l'"
...
sort idaluno
drop disciplina
save "$out\\Base 6EF `x' Sem Duplicados Merge", replace
}
local ano 2019 2020
foreach x of local ano{
clear all
use "$out\Base 6EF `x' Sem Duplicados Merge", clear
order codigo_escola
sort codigo_escola
tab periodo
gen turno_m=0
...
gen ErroCadastro = 0
 ...
 gen basenota = 2
drop quilombola
save "$out\\Base 6EF `x' Final Nova Iago", replace
}
clear all
use using "$out\\Base 6EF 2019 Final Nova Iago", clear
append using "$out\\Base 6EF 2020 Final Nova Iago"
...
save "\\ijsn625\ijsn\Projetos\Núcleo de Educação\9. Preditor de Abandono\Abandono 2020\iago\Global Out\base Lista 6EF 2020.dta", replace
keep if  anoref==2020 & classm7_15 == 1
save "\\ijsn625\ijsn\Projetos\Núcleo de Educação\9. Preditor de Abandono\Abandono 2020\iago\Global Out\base Lista 6EF 2020.dta", replace
clear all
cd "\\ijsn625\ijsn\Projetos\Núcleo de Educação\9. Preditor de Abandono\Abandono 2020\iago\Global Out"
use "base Lista 6EF 2020.dta"
keep sre codigo_escola escola municipio tipo_ensino periodo_letivo idaluno aluno tp_sexo idade classm7_15 nota_mt nota_lp nota_geo nota_his nota_cien prop_falta
rename sre SRE
